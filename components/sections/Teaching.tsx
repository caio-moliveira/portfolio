...
import { Play, ExternalLink, Radio, GraduationCap, PlayCircle } from "lucide-react";
import { useLang } from "@/lib/i18n";
import { Section } from "@/components/ui/Section";
import { Reveal } from "@/components/ui/Reveal";
import { videos, courses, teaching, socials } from "@/lib/content";
        ...
        {courses.map((c, i) => (
          <Reveal key={c.id} delay={i * 0.06}>
            ...
            >
              ...
              <h4 className="mt-4 font-display text-lg font-bold text-ink">{c.title[lang]}</h4>
              <p className="mt-2 flex-1 text-sm leading-relaxed text-muted">{c.desc[lang]}</p>
              <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-brand-bright transition-colors group-hover:text-cyan">
